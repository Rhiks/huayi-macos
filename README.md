...
**划到哪，翻到哪。** Huayi 把翻译放在正在阅读的文字旁边：学英语时随手查单词、短句并听发音；读 Paper 时翻术语和长段落；浏览器、PDF 阅读器、代码编辑器里都不用来回切换页面。

划选文字后，鼠标附近只出现一个 18×18 的小触点。需要译文时悬停展开，不需要时尽量少挡正文。短句保持低延迟，长文和技术语境可以交给本机 TranslateGemma 12B 流式处理。
...
| 学英语 | 读 Paper | 随手划译 |
| --- | --- | --- |
| 遇到生词或不熟悉的表达，划一下就看译文；系统语音可以直接朗读，查词不用离开原文。 | 技术术语、复杂句和长段落可交给本机 TranslateGemma 12B；译文边生成边显示，长文可以滚动阅读。 | 浏览器、PDF 阅读器和代码编辑器等可选文字场景都能使用；18×18 触点悬停展开，译文面板不主动抢键盘焦点。 |
